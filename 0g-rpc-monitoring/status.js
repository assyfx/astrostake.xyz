...
function formatBlockNumber(number) {
  return number.toLocaleString('en-US'); // 'en-US' untuk format internasional (koma ribuan)
}

// Fungsi untuk Update Status dari Backend
async function updateStatusFromBackend() {
  try {
    console.log("⏳ Memulai Fetch dari Backend...");

    const res = await fetch("https://astrostake.xyz/last-blocks", {
      method: "GET",
      headers: {
    ...
    if (!res.ok) {
      console.error("❌ Gagal Fetch Data dari Backend:", res.status);
      return;
    }
    ...
    document.querySelectorAll(".status-card").forEach(card => {
      ...
      console.log(`🔧 ${title} - Block dari Backend: ${rpcData.block}`);
...
function updateBlockChart() {
  const cards = Array.from(document.querySelectorAll('.status-card'));
  const labels = cards.map(c => c.querySelector('.status-title').textContent);
  const data = cards.map(c => {
    const txt = c.querySelector('.status-block').textContent;
    const m = txt.match(/Latest Block:\s*([\d,]+)/);
    return m ? parseInt(m[1].replace(/,/g, ""), 10) : 0;
  });

  const valid = data.filter(b => b>0);
  if (!valid.length) return;
  const max = Math.max(...valid);
  const minCandidate = Math.min(...valid);
  const min = Math.max(minCandidate - 5, max - 50);
  ...
  data.forEach((val,i) => {
    ...
    fData.push(val);
    fColors.push(gap>1?'#facc15':'#4d61ff');
  });

  blockChart.data.labels = fLabels;
  blockChart.data.datasets[0].data = fData;
  blockChart.data.datasets[0].backgroundColor = fColors;
  blockChart.options.scales.y.min = min===max? max-10 : min;
  blockChart.options.scales.y.max = max+10;
